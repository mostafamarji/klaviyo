...
def get_next_data(sincedata, fromdate, todate, requestname):
    # try:
    ...
    # open_email
    # metrics = client.Metrics.get_metric_timeline_by_id("Nk7mzP", since=sincedata, sort="desc")

    # droped_email
    # metrics = client.Metrics.get_metric_timeline_by_id("LUVvDf", since=sincedata, sort="desc")

    r = json.dumps(metrics.data["data"])
    loaded_r = json.loads(r)
    # print(loaded_r)
    # save_dropped_email_data_big_query(loaded_r, table, fromdate, todate)
    # save_open_data_big_query(loaded_r, table, fromdate, todate)
    # save_click_data_big_query(loaded_r, table, fromdate, todate)
    save_bounce_data_big_query(loaded_r, table, fromdate, todate)

    global allow_get_next_data
    if (metrics.data["next"] != "") & allow_get_next_data:
        time.sleep(3)
        print("Getting next data")
        get_next_data(metrics.data["next"], fromdate, todate, requestname)
    else:
        return
...
def save_dropped_email_data_big_query(loaded_r, table, fromdate, todate):
    global allow_get_next_data
    try:
        for i in loaded_r:
            ...
            if to_date <= record_date <= from_date:
                item = {
                    'event_properties':
                        {
...
def save_click_data_big_query(loaded_r, table, fromdate, todate):
    global allow_get_next_data
    try:
        for i in loaded_r:
            record_date = datetime.fromisoformat(i["datetime"]).date()
...
def save_open_data_big_query(loaded_r, table, fromdate, todate):
    global allow_get_next_data
    try:
        for i in loaded_r:
            record_date = datetime.fromisoformat(i["datetime"]).date()
...
def save_bounce_data_big_query(loaded_r, table, fromdate, todate):
    global allow_get_next_data
    try:
        for i in loaded_r:
            record_date = datetime.fromisoformat(i["datetime"]).date()
