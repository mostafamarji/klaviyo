...
def get_next_data(sincedata, requestname):
    ...
    metrics = client.Metrics.get_metric_timeline_by_id("K8vC8L", since=sincedata)
    r = json.dumps(metrics.data["data"])
    loaded_r = json.loads(r)

    save_data_big_query(loaded_r, table)
...
def save_data_big_query(loaded_r, table):
    try:
        for i in loaded_r:
            item = {
                'event_properties':
